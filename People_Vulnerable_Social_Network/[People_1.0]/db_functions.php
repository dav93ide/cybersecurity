<?php
	...
	function new_user($name,$pass,$email,$birthdate,$first_name,$last_name,$sex,$secret_question,$secret_answer){
		$db = new mysqli('localhost:3306','root','','people_db');
	...
	function check_follower($name,$otherName){
		$db = new mysqli('localhost','root','','people_db');
		$idUser = get_id_from_name($name);
	...
	function add_img($name,$img,$img_name,$profile,$mex){
		$db = new mysqli('localhost','root','','people_db');
		$iduser = get_id_from_name($name);
		if($img['size'] > 1800000 || $img['size'] == 0){
			return "big";
		}
		if(getimagesize($img)){
			return "noimg";
		...
		do{
			$imname = strval(time()) . "-" . $img_name . "-" . strval(rand(0,999));
			$ris = $db -> query("SELECT id FROM imglist WHERE img_link like '%$imname%'");
		} while($ris->num_rows > 0);
		$img_link = "data/$name/$imname". substr($img['name'],-4);
		move_uploaded_file( $img["tmp_name"] , $img_link);
		$db -> query("INSERT INTO imglist(img_link) VALUES ('$img_link')");
		$ris = $db -> query("SELECT id FROM imglist WHERE img_link = '$img_link'");
